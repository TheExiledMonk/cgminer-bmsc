...
static void set_bmsc_speed(int set_mhz)
{
int f=0;
int n = 0, min=0;
float K[4];        //M+1 placeholder
int rK[4];         //rounded values of M+1
float dK[4];       //M+1 integer tolerance (rounding error)
int M, N, OD, BS;  //values for frequency calculation
int hex_res;   //for binary concatenation
float actual_clock = 0;
int timeout = 0;
int i = 0;
if( f >= 300 ) i = 1; //no real reason, just consistency with BM stock values
min = i;
char bmsc_options;
for(i; i < 4; i++ ) 
{
  K[i] = ( f * pow(2,i) )/25;
 //actual K
  rK[i] = (10*K[i] + 5)/10; //rounded integer K
  dK[i] = abs((K[i] / rK[i])-1); //store the rounding tolerance
   if( dK[i] < dK[min] ) min = i; //will always keep the lowest index
                                 //on repeated minimum values
   } M=rK[min]-1;
//get your M from K=M+1
 if(min==3)
{
   N = 1;
   OD = 2;
...
//get your N and OD
 actual_clock= (25*(M+1))/((N+1)*pow(2,OD));
 //make sure BS criteria is met
 if( (pow(2,OD)*actual_clock)<300 )
{
      //if 2^OD*f is too low
   OD++;
//double 2^OD and double M+1
   M= ((M+1)*2)-1;
}
 if( (pow(2,OD)*actual_clock) > 1000 )
{
   //if 2^OD*f is too high
   OD--;
  //halve 2^OD and halve M+1
   if( (M+2)/2 > (M+1)/2 )
{

    if( abs(f-(25*M/(2*(N+1)*pow(2,OD)))) < abs(f-(25*(M+2)/(2*(N+1)*pow(2,OD)))) )
      M=M/2 - 1;   //use M/2 (which is to say, X)
    else
      M=M/2;  //use M/2 + 1 (which is to say, X+1)
   }else //m+1 is even
    M=(M+1)/2 ;
 ...
 //recalculate actual_clock with new values; hopefully BS criteria is still met 
 actual_clock= (25*(M+1))/((N+1)*pow(2,OD));

 
 //set BS
 if( (pow(2,OD)*actual_clock) >= 300 && (pow(2,OD)*actual_clock) <= 600 ) BS=0;
 if( (pow(2,OD)*actual_clock) > 600 && (pow(2,OD)*actual_clock) <= 1000 ) BS=1;
 if( (pow(2,OD)*actual_clock) > 1000 )
    {
    printf("fatal error, try another Mhz value\n");
    exit(254);
    }
 ...
 //bit shift and concatenate binary values
 hex_res  = (BS<<14) + (M<<7) +(N<<2) + OD;

 
 //print hex output
 printf( "Hex value: %04x\n", hex_res);
 //print actual clock value generated by calculated hex value
 printf( " Resulting actual clock: %.2f\n", actual_clock );
 //print percent error of actual_clock from entered frequency
 printf( " Error from desired clock: %.2f \%\n", 100*((actual_clock/f)-1) );
 //print expected hashrate based on actual clock
 printf( " Expected hashrate output: %.2f GH/s\n", actual_clock*0.512 );


 //best guess for timeout = 13800/clock, round down to nearest 5
 timeout = 5*((27600/actual_clock)+5 / 10);
 printf(" Best estimate timeout: %i ", timeout);
 
bmsc_options = "115200:" + timeout/100;
// set baud speed as default 115200
set_bmsc_options(bmsc_options);
// set timeout per calculated
set_bmsc_freq(hex_res);
}
